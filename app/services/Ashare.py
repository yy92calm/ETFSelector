...
import json,requests,datetime,logging;      import pandas as pd  #

_logger = logging.getLogger(__name__)
# 统一请求超时（秒）：连接10s、读取15s，避免部署环境网络受限时无限挂起
_REQ_TIMEOUT = (10, 15)


def _http_get(url):
    """带超时的 GET，返回解析后的 JSON；失败抛异常给上层 try/except 处理"""
...
def get_price_day_tx(code, end_date='', count=10, frequency='1d'):     #日线获取
    ...
    st= _http_get(URL);    ms='qfq'+unit;      stk=st['data'][code]
...
def get_price_min_tx(code, end_date=None, count=10, frequency='1d'):    #分钟线获取
    ...
    st= _http_get(URL);       buf=st['data'][code]['m'+str(ts)]
...
def get_price(code, end_date='',count=10, frequency='1d', fields=[]):        #对外暴露只有唯一函数
    ...
    if  frequency in ['1d','1w','1M']:   #1d日线  1w周线  1M月线
         try:    return get_price_sina( xcode, end_date=end_date,count=count,frequency=frequency)   #主力
             ...
             _logger.warning(f"[Ashare] sina 日线失败，降级腾讯: {e}")
    ...
    if  frequency in ['1m','5m','15m','30m','60m']:  #分钟线
         ...
         try:    return get_price_sina(  xcode,end_date=end_date,count=count,frequency=frequency)   #主力
             ...
             _logger.warning(f"[Ashare] sina 分钟线失败，降级腾讯: {e}")
