...
注意：最多建议替换2只。如果持仓都足够强，可以建议不换。若标的带有 industry_value 字段（行业盈利-估值性价比排名，rank 越小行业越值钱），可作为趋势可持续性的板块层面佐证，严禁由其得出任何个股结论。规则参考是历史统计偏好：当它与动量判断同向时可用于加强论据；当它反对某笔换入时，必须给出更强的趋势证据再坚持换入。"""
