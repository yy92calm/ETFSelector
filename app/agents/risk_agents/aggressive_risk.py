...
class AggressiveRiskAgent(BaseAgent):
    ...
    PROMPT = """你是激进型风控官。你的风控哲学是：**市场长期向上，短期回调是机会，敢于在下跌中加仓**。
你倾向于容忍较高的波动和回撤，追求更高收益。
...
## 决策要求
基于你的激进风控哲学，输出JSON格式的风控意见（不要包含其他文字）：
{{
  "risk_philosophy": "aggressive",
...
position_adjustment: 建议的仓位调整系数，1.0=满仓，0.5=半仓，0.0=空仓"""
    ...
    def analyze(self, risk_data: Dict) -> Dict:
        ...
        return result if result else {"error": "AggressiveRiskAgent分析失败"}
