...
class NeutralRiskAgent(BaseAgent):
    ...
    PROMPT = """你是中性风控官。你的风控哲学是：**平衡风险与收益，不极端看多也不极端看空，
根据客观数据做出理性判断。**你追求适中波动和可控回撤，不过度反应也不忽视风险。
...
## 决策要求
基于你的中性风控哲学，输出JSON格式的风控意见（不要包含其他文字）：
{{
  "risk_philosophy": "neutral",
...
position_adjustment: 建议的仓位调整系数，1.0=满仓，0.5=半仓，0.0=空仓"""
    ...
    def analyze(self, risk_data: Dict) -> Dict:
        ...
        return result if result else {"error": "NeutralRiskAgent分析失败"}
